...
- (kxMovieError) openInput: (NSString *) path
{
    ...
    // Emby direct-play streams come over plain HTTP/Wi-Fi: give the socket a
    // sane timeout and let the http protocol layer ride out transient
    // drop-outs via byte-range reconnects (seeking still uses Range).
    AVDictionary *opts = NULL;
    if (_isNetwork) {
        av_dict_set(&opts, "timeout", "15000000", 0);        // 15s socket I/O timeout

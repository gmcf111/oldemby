...
- 视频播放不强制使用系统播放器：允许引入第三方播放器内核（FFmpeg 系，如 kxmovie 架构）以实现 MKV 等封装的本地直接播放。系统播放器 `MPMoviePlayerViewController` 仍作为 mp4/mov 等原生兼容格式的路径；`AVPlayerViewController` 仍不可用（iOS 8+）。第三方内核必须自行支持 armv7 / iOS 6 渲染（OpenGL ES + AudioQueue），且其依赖库在 CI 中以 9.3 SDK、`-miphoneos-version-min=6.0` 构建。
...
本仓库没有本地测试框架、lint 任务或可单独运行的测试。Windows 开发环境只编辑源码；不要在本地执行 `make`。唯一受支持的构建验证是在 GitHub Actions 的 **OldEmby Build** workflow 中完成：它下载 Theos、iOS 工具链和 SDK，用 `tools/build_ffmpeg.sh` 构建 armv7/iOS 6 的 FFmpeg 静态库（缓存于 `vendor/ffmpeg`，产物不入库），执行 `make package`，修补 iOS 6 dyld 绑定，并生成 `.ipa` 与 `.deb`。
...
- `Sources/Services/OEEmbyAPIClient` 是所有 Emby HTTP 请求的唯一入口，负责认证、分页媒体查询、`PlaybackInfo` 与播放 URL。所有回调运行于主队列；请求和播放流程的改动应保留其现有错误处理与 URL 编码逻辑。视频 direct 请求完成值为 `@{url, useFFmpeg}` 字典：`useFFmpeg` 标记系统播放器无法打开的媒体（MKV/AVI/AC3/10-bit 等），详情页据此路由到内置 FFmpeg 播放器；音频仍是纯 URL 字符串。
- `Sources/Player/`（kxmovie 核心，LGPL v3，见 `lgpl-3.0.txt`）+ `Sources/Controllers/OEFFmpegPlayerViewController` 构成第二播放内核：FFmpeg 直接拉取 `Static=true` 原始文件 URL 做解封装与软解，实现 MKV 等容器的真·不转码播放（HEVC/VP9/AV1 与 4K 除外，仍引导走转码）。UI 复用 `OESubtitleOverlayView` + 服务器 SRT 端点；音轨通过 `KxMovieDecoder.selectedAudioStream` 在解码队列上切换。mp4/mov 等原生格式继续用 `MPMoviePlayerViewController`（硬解、省电）。
...
`.github/workflows/build.yml` 是构建行为的权威来源。它先用 `tools/patch_sdk_tbd.py` 修复 Theos SDK stub，再用 `tools/build_ffmpeg.sh` 产出 armv7/iOS 6 的 FFmpeg 静态库（`vendor/ffmpeg`，actions/cache 按脚本 key 复用），构建 armv7 app，解析实际的 `OldEmby.app` 路径，对同一二进制运行 `tools/fix_ios6_bindings.py` 并重签，然后组装 IPA。更改构建、SDK 或打包相关文件时，须保留这些顺序依赖和 IPA 的 `Info.plist`、图标、Mach-O、无 entitlements 校验。FFmpeg 以 `--disable-encoders/muxers/filters` 等裁剪，NEON 构建失败时自动回退 `--disable-asm` 纯 C 解码。
